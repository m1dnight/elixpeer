defmodule Elixpeer.TorrentsTest do
  ...
  describe "insert_or_update_torrent" do
    test "insert new torrent" do
      # upsert the torrent
      result = Torrents.upsert(@torrent_map)
    ...
    test "upsert does not change id" do
      # upsert the torrent
      result1 = Torrents.upsert(@torrent_map)
      assert Torrents.list() == [result1]
      assert Enum.sort(Trackers.list()) == Enum.sort(result1.trackers)

      result2 = Torrents.upsert(@torrent_map)
